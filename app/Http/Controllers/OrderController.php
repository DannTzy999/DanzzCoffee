...
class OrderController extends Controller
{
    ...
    public function makeOrderPost(Request $request, Product $product)
    {
        $rules = [
            ...
            'proof_payment' => 'nullable|image|max:2048'
        ];
        ...
        $message = [
            ...
            'proof_payment.image' => 'Proof payment must be an image',
            'proof_payment.max' => 'Proof payment must not exceed 2MB'
        ];

        $validatedData = $request->validate($rules, $message);

        try {
            // Handle proof payment file
            $transaction_doc = env("IMAGE_PROOF");
            if ($request->hasFile("proof_payment") && $request->file("proof_payment")->isValid()) {
                $transaction_doc = $request->file("proof_payment")->store("proof");
                // Update note to indicate proof uploaded
                $note_id = 3; // Note for proof uploaded
            } else {
            ...
            $data = [
                ...
                "note_id" => $note_id,
                "status_id" => 2,
                "transaction_doc" => ($validatedData["payment_method"] == 1) ? $transaction_doc : null,
                "is_done" => 0,
    ...
    public function orderData()
    {
        ...
        if (auth()->user()->role_id == Role::ADMIN_ID) {
            $orders = Order::with("note", "payment", "user", "status", "product")->where(["is_done" => 0])->orderBy("id", "ASC")->get();
        } else {
            $orders = Order::with("note", "payment", "user", "status", "product")->where(["user_id" => auth()->user()->id, "is_done" => 0])->orderBy("id", "ASC")->get();
        }
        $status = Status::all();
    ...
    {
        $title = "Order Data";
        $orders = Order::with("note", "payment", "user", "status", "product")->where("status_id", $status_id)->orderBy("id", "ASC")->get();
        $status = Status::all();
    ...
    {
        $order->load("product", "user", "note", "status", "payment");
        return $order;
    }
    ...
    public function orderHistory()
    {
        $title = "History Data";
        if (auth()->user()->role_id == Role::ADMIN_ID) {
            $orders = Order::with("note", "payment", "user", "status", "product")->where(["is_done" => 1])->orderBy("id", "ASC")->get();
        } else {
            $orders = Order::with("note", "payment", "user", "status", "product")->where(["user_id" => auth()->user()->id, "is_done" => 1])->orderBy("id", "ASC")->get();
        }
        $status = Status::all();
